...
# FIRE
fire=read.table('../HiC_results_yyc/FIRE_ANALYSIS_40KB.Schmitt_IMR90.txt', head=T)
gr.i=GRanges(seqnames=fire$chr, range=IRanges(start=fire$start,end=fire$end), score=fire$IMR90_40Kb_raw_count_matrix_FIRES, fire=fire$IMR90_40Kb_raw_count_matrix_indicator)
table(width(gr.i))
...
table(gr.i$fire)
binom.test(66,66+219, 4428/(4428+59495), alternative='greater')
...
plot(density(pboot), xlim=c(0,0.24)); abline(v=66/(66+219), lty=2)
sum(66/(66+219)<=pboot)/length(pboot)
...
table(gr.i$fire)
binom.test(3,93, 4428/(4428+59495), alternative='less')
...
plot(density(pboot), xlim=c(0,0.23));  abline(v=3/93, lty=2)

sum(3/93>=pboot)/length(pboot)
...
hist(pboot)
plot(density(pboot), xlim=c(0,0.06));  abline(v=17/331, lty=2)
...
hist(pboot)
plot(density(pboot), xlim=c(0,0.06));  abline(v=0, lty=2)
...
# load in significant interactions
