...
colnames(XR_TS)=paste(XR_samp$damage,'_', XR_samp$genotype,'_',XR_samp$timepoint,'_', XR_samp$replicate,sep='')
XR_NTS=XR_TS

for(i in 1:nrow(XR_samp)){
  cat(i,'\t')
  temp.plus=read.table(paste('../output/XR_',XR_samp$damage[i],'_',XR_samp$genotype[i],'_',XR_samp$timepoint[i],'_',XR_samp$replicate[i],'_XRseq_gene_plus.txt',sep=''))[,2]
  temp.minus=read.table(paste('../output/XR_',XR_samp$damage[i],'_',XR_samp$genotype[i],'_',XR_samp$timepoint[i],'_',XR_samp$replicate[i],'_XRseq_gene_minus.txt',sep=''))[,2]
...
# The number of TT dimers are highly correlated with gene length
...
pdf(file='../output/gene_TS_NTS.pdf',width=13, height=8)
par(mfrow=c(3,6))
for(i in 1:nrow(XR_samp)){
  plot(XR_TS[,i],XR_NTS[,i], pch=16, cex=0.5,
       xlim=c(0,max(c(XR_TS[,i],XR_NTS[i,]))), ylim=c(0,max(c(XR_TS[,i],XR_NTS[i,]))),
       xlab='XR_TS',ylab='XR_NTS',
       main=paste0(XR_samp$genotype[i],'_',XR_samp$timepoint[i],'_',XR_samp$replicate[i])); abline(a=0,b=1,col=2, lty=2, lwd=2)
...
TCR.output=XR_samp[,c('damage','genotype','timepoint','replicate','TCR.lower.quartile','TCR.median','TCR.upper.quartile')]
...
annotation = data.frame(damage=XR_samp$damage,Time = XR_samp$timepoint, replicate=XR_samp$replicate)
rownames(annotation) = rownames(XR.cor)=colnames(XR.cor)=gsub('_xpc','',colnames(XR.RPKM))
pdf(file='../output/pairwise_cor_pheatmap.pdf',width=7, height=6)
...
v1=data.frame(PC1=XR_TS.svd$v[,1], PC2=XR_TS.svd$v[,2], timepoint=XR_samp$timepoint, 
              damage=XR_samp$damage,genotype=XR_samp$genotype)
v1$genotype_timepoint=paste0(v1$genotype,'_',v1$timepoint)
p <- ggplot(v1, aes(PC1, PC2, label=genotype_timepoint)) + geom_point(aes(colour=genotype, shape=genotype)) +
  geom_text_repel(aes(PC1, PC2, label=timepoint), max.overlaps = 30, size=3) + ggtitle("RPKM of TS repair") 
p
ggsave(p, file='../output/svd.pdf', width=5.5, height=4.5)

# between two replicates
pdf(file='../output/replicates_cor.pdf', width=8, height=8)
par(mfrow=c(3,3))
for(i in which(XR_samp$replicate=='rep2')){
  plot.x=XR[,i-1]/(sum(XR[,i-1])/10^6)
  plot.y=XR[,i]/(sum(XR[,i])/10^6)
  plot(plot.x,plot.y, pch=16, cex=0.5,
       xlab=paste(XR_samp$genotype[i],XR_samp$timepoint[i],'rep1'),
       ylab=paste(XR_samp$genotype[i],XR_samp$timepoint[i],'rep2'),
       main=paste0(XR_samp$genotype[i],'_',XR_samp$damage[i],'_',XR_samp$timepoint[i]),
