...
sampinfo=as.data.frame(readxl::read_excel('../new_bams/sample_info_new.xlsx'))
sampinfo$sample_name=gsub('_trimmed_sorted.bam','', sampinfo$sample_name)
sampinfo$timepoint=factor(sampinfo$timepoint,
...
for(i in 1:nrow(sampinfo)){
  ...
  cat('\n', i, '\t', bamnamei,'\n')
  bamPath=paste('../new_bams/',bamnamei,'_trimmed_sorted.bam',sep='')
  ...
  pdf(file=paste('../output/XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'_readlength_hist.pdf',sep=''), width=4, height=3)
  hist(width(gr),breaks=0:50,xlab='Read length',
       main=paste0('XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],sep=''),xlim=c(10,40))
  dev.off()
  ...
  pdf(paste('../output/XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'_dinucleotide_freq.pdf',sep=''),width=8,height=8)
  ...
  title(paste0('XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'+ strand, read length',gr.plus.qwidthi$qwidth[1]))
  ...
  if(sampinfo$damage[i]=='CPD'){
    ...
    title(paste('XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'+ strand TT freq, read length',qwidthi))
    ...
    title(paste('XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'+ strand TC+TT freq, read length',qwidthi))
  }
  dev.off()
  save(gr, file=paste('../output/XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'_gr_qc.rda',sep=''))
  ...
  save(reads,file=paste('../output/XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'_reads.rda',sep=''))
  save(gr, file=paste('../output/XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'_gr_qc_gene.rda',sep=''))
  ...
  write.table(plus.strand, file=paste('../output/XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'_XRseq_gene_plus.txt',sep=''),col.names = F, row.names = T, sep='\t', quote = F)
  write.table(minus.strand, file=paste('../output/XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'_XRseq_gene_minus.txt',sep=''),col.names = F, row.names = T, sep='\t', quote = F)
...
i=1
load(paste('../output/XR_',sampinfo$damage[i],'_',sampinfo$genotype[i],'_',sampinfo$timepoint[i],'_',sampinfo$replicate[i],'_reads.rda',sep=''))
...
write.csv(sampinfo, file='../output/sampinfo_with_reads.csv', row.names = F)

sampinfo$genotype_timepoint=paste0(sampinfo$genotype,'_',sampinfo$timepoint)

p <- ggplot(sampinfo, aes(log(total_mapped,10), log(qwidth,10), label = genotype_timepoint))
p + geom_point(aes(colour = genotype, shape=damage)) + geom_text(vjust = 0, nudge_y = 0.02) +
